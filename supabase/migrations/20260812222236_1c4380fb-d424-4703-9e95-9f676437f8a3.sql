-- F12.1-B — seleção humana de KPIs + fonte não oficial + mapeamento seguro de pilares.
-- Aditivo e idempotente. Nenhum dado é criado ou alterado.

CREATE TABLE IF NOT EXISTS public.strategy_recommendation_kpi_decisions (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  organization_id uuid NOT NULL,
  business_unit_id uuid NOT NULL,
  template_objective_id uuid NOT NULL REFERENCES public.strategy_template_objectives(id),
  template_kpi_id uuid NOT NULL REFERENCES public.strategy_template_kpis(id),
  decision text NOT NULL CHECK (decision IN ('accepted', 'discarded')),
  applied_kpi_id uuid NULL REFERENCES public.kpis(id),
  applied_at timestamptz NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  created_by uuid NULL REFERENCES public.users(id),
  updated_at timestamptz NOT NULL DEFAULT now(),
  updated_by uuid NULL REFERENCES public.users(id),
  CONSTRAINT strategy_rec_kpi_dec_bu_fk
    FOREIGN KEY (business_unit_id, organization_id)
    REFERENCES public.business_units (id, organization_id),
  CONSTRAINT strategy_rec_kpi_dec_unique UNIQUE (business_unit_id, template_kpi_id)
);

CREATE INDEX IF NOT EXISTS strategy_rec_kpi_dec_bu_obj_idx
  ON public.strategy_recommendation_kpi_decisions (business_unit_id, template_objective_id);

GRANT SELECT, INSERT, UPDATE ON public.strategy_recommendation_kpi_decisions TO authenticated;
GRANT ALL ON public.strategy_recommendation_kpi_decisions TO service_role;
REVOKE ALL ON public.strategy_recommendation_kpi_decisions FROM anon;

ALTER TABLE public.strategy_recommendation_kpi_decisions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "kpi_decisions_select" ON public.strategy_recommendation_kpi_decisions;
CREATE POLICY "kpi_decisions_select"
  ON public.strategy_recommendation_kpi_decisions FOR SELECT TO authenticated
  USING (
    public.has_permission('strategy.read'::public.citext, 'business_unit',
      public.f2_bu_scope_id(business_unit_id))
  );

DROP POLICY IF EXISTS "kpi_decisions_insert" ON public.strategy_recommendation_kpi_decisions;
CREATE POLICY "kpi_decisions_insert"
  ON public.strategy_recommendation_kpi_decisions FOR INSERT TO authenticated
  WITH CHECK (
    public.has_permission('strategy.manage'::public.citext, 'business_unit',
      public.f2_bu_scope_id(business_unit_id))
  );

DROP POLICY IF EXISTS "kpi_decisions_update" ON public.strategy_recommendation_kpi_decisions;
CREATE POLICY "kpi_decisions_update"
  ON public.strategy_recommendation_kpi_decisions FOR UPDATE TO authenticated
  USING (
    public.has_permission('strategy.manage'::public.citext, 'business_unit',
      public.f2_bu_scope_id(business_unit_id))
  )
...
DROP TRIGGER IF EXISTS trg_strategy_rec_kpi_dec_touch ON public.strategy_recommendation_kpi_decisions;
CREATE TRIGGER trg_strategy_rec_kpi_dec_touch
  BEFORE UPDATE ON public.strategy_recommendation_kpi_decisions
  FOR EACH ROW EXECUTE FUNCTION public.f1_touch_updated_at();

DROP TRIGGER IF EXISTS trg_strategy_rec_kpi_dec_audit ON public.strategy_recommendation_kpi_decisions;
CREATE TRIGGER trg_strategy_rec_kpi_dec_audit
  AFTER INSERT OR UPDATE ON public.strategy_recommendation_kpi_decisions
  FOR EACH ROW EXECUTE FUNCTION public.f2_audit();

-- Aliases determinísticos e explícitos por dimensão (sem fuzzy matching).
CREATE OR REPLACE FUNCTION public.f12_dimension_pillar_aliases(p_dimension text)
RETURNS text[]
LANGUAGE sql
IMMUTABLE
SET search_path TO ''
AS $$
  SELECT CASE p_dimension
    WHEN 'finance' THEN ARRAY['finanças','financeiro','financeira','gestão administrativa & financeira']
    WHEN 'marketing_sales' THEN ARRAY['marketing e vendas','comercial','vendas','gestão comercial & relacionamento']
    WHEN 'operations' THEN ARRAY['operações','operacional','processos','gestão produtiva, lavra & manutenção']
    WHEN 'people' THEN ARRAY['pessoas','recursos humanos','gestão de pessoas, tecnologia & suporte']
    WHEN 'governance' THEN ARRAY['governança','direção e governança']
    ELSE ARRAY['direção estratégica']
  END
$$;

REVOKE ALL ON FUNCTION public.f12_dimension_pillar_aliases(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.f12_dimension_pillar_aliases(text) TO authenticated, service_role;
...
BEGIN
  ...
  LOOP
    ...
    -- A) pilar existente equivalente (lista fechada de aliases), no próprio ciclo
    ...
    -- Somente KPIs explicitamente aceitos e ainda não aplicados.
    -- source, owner, baseline e meta continuam decisões humanas do F8.
    FOR v_kpi IN
